...
class DRMM_TKS(utils.SaveLoad):
    ...
    def _get_keras_model(self, embed_trainable=False, dropout_rate=0.5, hidden_sizes=[100, 1]):
        ...
        passage_question_vector = Lambda(lambda x: K.sum(K.expand_dims(x[0], axis=-1) * x[1], -2)) ([passage_question_attention, encoded_question])
        ...
        repeat_layer = Repeat(axis=1, copy_from_axis=1)
        # Shape: (batch_size, num_passage_words, embedding_dim * 2)
