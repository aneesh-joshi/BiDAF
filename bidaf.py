...
bidir_encoder = Bidirectional(LSTM(embedding_dim, return_sequences=True), merge_mode='concat')

encoded_question = bidir_encoder(question_embedding)
encoded_passage = bidir_encoder(passage_embedding)
...
for i in range(num_hidden_ending_bidir_layers):
    hidden_layer = Bidirectional(LSTM(embedding_dim, return_sequences=True, name="hidden_seq2seq_{}".format(i)), merge_mode='concat')
